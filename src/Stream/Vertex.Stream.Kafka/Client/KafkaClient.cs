...
namespace Vertex.Stream.Kafka
{
    public class KafkaClient : IKafkaClient
    {
        ...
        {
            this.producerObjectPool = new DefaultObjectPool<PooledProducer>(new PooledProducerPolicy(producerConfig.Value, logger), options.Value.ProducerMaxPoolSize);
            ...
            var consumerObjectPool = this.consumerPoolDict.GetOrAdd(group, key =>
             {
                 ...
                 return new DefaultObjectPool<PooledConsumer>(new PooledConsumerPolicy(config, this.logger), this.options.ConsumerMaxPoolSize);
             });
